...
| File | Purpose |
|---|---|
| `index.html` | Home page — hero banner, value props, featured treats, brand story preview, testimonials, promo, Instagram feed, newsletter sign-up |
| `about.html` | Full brand story |
| `shop.html` | Full product menu |
| `product-detail.html` | Individual product page (expects a `?id=` query parameter, e.g. `cupcake-box-6`) |
| `custom-orders.html` | Custom cake / celebration order enquiries |
| `corporate.html` | Corporate & bulk order info |
...
2. Make sure the `assets/` and `css/` folders sit alongside the HTML files as shown above.
3. Open `index.html` in a browser, or serve the folder with a local dev server (e.g. VS Code's Live Server extension) for the best results.
